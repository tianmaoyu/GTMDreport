(function ($) {
    ...
    function createNewRow(row, fn, param) {
        ...
        function createRowFromUrl() {
            xt.ajax($.easyui.newRowUrl, param, {
                type: "get",
    ...
    function save(grid, url, callback, fnSaveBefore, fnRefresh, fnFilter) {
        ...
        if (changes && changes.length == 0) {
            $.easyui.info($.easyui.saveNotChangeMessage);
            return;
        }
        ...
        xt.ajax(url || $.easyui.saveUrl, { addList: $.toJSON(addList), updateList: $.toJSON(updateList), deleteList: getDeleteId(deleteList) }, {
            success: function (json) {
    ...
    function submitIds(grid, url, callback, fnSubmitBefore, fnRefresh) {
        ...
        if (!rows || $.isEmptyArray(rows)) {
            rows = grid.datagrid("getChecked");
            if (!rows || $.isEmptyArray(rows)) {
                $.easyui.info($.easyui.notCheckedMessage);
                return;
            }
        }

        xt.ajax(url || $.easyui.submitIdsUrl, { idList: $.easyui.rowConvert(rows) }, {
            success: function (json) {
                $.easyui.showFormMessage(json, function () {
                    if (fnRefresh)
                        fnRefresh(json);
    ...
    $.easyui.grid = function () {
        return {
            ...
            getPagerTotal: function (grid) {
                ...
                return $.toNumber(pager.pagination('options').total);
            },
            //更新总行数
            ...
            edit: function (gridId) {
                ...
                if (!row) {
                    $.easyui.info($.easyui.editNotSelectedMessage);
                    return;
                }
            ...
            deleteById: function (gridId) {
                ...
                if (result && result.length > 0) {
                    $.easyui.confirm($.easyui.deleteConfirmMessageForSave, deleteChecked());
                    return;
                }
                var selectedRow = grid.datagrid('getSelected');
                if (!selectedRow) {
                    $.easyui.info($.easyui.deleteNotSelectedMessage);
                    return;
                }
                $.easyui.confirm($.easyui.deleteConfirmMessageForSave, function () { grid.edatagrid("destroyRow"); });
    ...
    $.easyui.treegrid = function () {
        ...
        return {
            ...
            deleteById: function (gridId) {
                ...
                if (ids.length > 0) {
                    $.easyui.confirm($.easyui.deleteConfirmMessageForSave, deleteCheckedRows());
                    return;
                }
                ...
                if (!$.isEmptyArray(grid.etreegrid("getChildren", selectedRow.Id))) {
                    $.easyui.info("不能删除有子节点的父节点，请重新选择");
                    return;
                }
                $.easyui.confirm($.easyui.deleteConfirmMessageForSave, function () { grid.etreegrid("destroyRow", selectedRow.Id); });
            ...
            moveUp: function (gridId) {
                ...
                if (!row) {
                    $.easyui.info($.easyui.moveUpNotSelectedMessage);
                    return;
                }
            ...
            moveDown: function (gridId) {
                ...
                var row = grid.treegrid("getSelected");
                if (!row) {
                    $.easyui.info($.easyui.moveDownNotSelectedMessage);
                    return;
                }
